<?php
    ...
    $store = $_POST['store'];
    $query = "SELECT name,price,`order`.quantity,`order`.id'id',weight,user_id,`order`.created_at'date' FROM product join `order` WHERE product.id = `order`.product_id and product.store_id = $store and `order`.`status` = 3 order by `order`.created_at desc";
