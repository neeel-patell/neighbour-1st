<?php 
    ...
    $query = "insert into vendor(first_name,last_name,mobile,email,password,gender,dob,address,area_id) values
    ('$first_name','$last_name',$mobile,'$email','$password',$gender,'$date_of_birth',\"$address\",$area)";
    if($conn->query($query) == true){
        $image = $_POST['image'];
        $query = "select id from vendor where mobile = $mobile";
        $result = $conn->query($query);
        $row = $result->fetch_array();
        $id = $row['id'];
        $path = "../images/profile/vendor/$id.jpg";
